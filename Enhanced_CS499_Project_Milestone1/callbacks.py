...
def register_callbacks(app, animal_service):
    ...
    def update_dashboard(filter_type: str):
        try:
            data, columns = animal_service.get_table_data(filter_type)
            return data, columns, f"Displaying {len(data)} matching animal record(s)."
        except ValidationError as exc:
            return no_update, no_update, str(exc)
        except DatabaseOperationError:
            return no_update, no_update, "Animal records could not be loaded."
